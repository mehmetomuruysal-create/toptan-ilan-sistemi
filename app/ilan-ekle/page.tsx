...
import { Clock } from "lucide-react"; // Eksik olan buydu
import Link from "next/link"; // Eksik olan buydu
...
export default async function IlanEklePage() {
  ...
  // 1. DURUM: ONAYLI KULLANICI -> GERÇEK 3 ADIMLI FORMU GÖSTER
  ...
  // 2. DURUM: BELGELERİ YÜKLEMİŞ AMA ONAY BEKLEYEN KULLANICI
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 text-center border border-gray-100">
          ...
          <p className="text-gray-500 font-medium text-sm mb-8">
            Belgeleriniz başarıyla yüklendi. Yönetici onayından sonra ilan verebilirsiniz.
          </p>
          <Link 
            href="/" 
            className="block w-full bg-gray-900 text-white py-4 rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-black transition-all"
          >
            Ana Sayfaya Dön
          </Link>
        </div>
  ...
  // 3. DURUM: HENÜZ BELGE YÜKLEMEMİŞ KULLANICI -> MODAL'I GÖSTER
  return <BelgeOnayModal userId={userId} />;
}